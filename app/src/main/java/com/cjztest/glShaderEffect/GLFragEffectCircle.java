...
public class GLFragEffectCircle extends GLLine {
    ...
    private int mTargetPointer;

    public GLFragEffectCircle(int baseProgramPointer, float x, float y, float z, float w, float h, int windowW, int windowH, Context context) {
    ...
    private void startBindTexture() {
        //特殊纹理，需要专门加载其他程序:
        String fragShaderScript = ShaderUtil.loadFromAssetsFile("fragColorEffect1/fragShaderCircle.shader", mContext.getResources());
        ...
        mCircleProgram = GLES30.glCreateProgram();
        //若程序创建成功则向程序中加入顶点着色器与片元着色器
        if (mCircleProgram != 0) { //step 1: 创建program后附加编译后的脚本
            //>>>>>>>>>>>>
            //向程序中加入顶点着色器
            GLES30.glAttachShader(mCircleProgram, mWaveFragShaderPointer);
            checkGlError("glAttachShader");
            //向程序中加入片元着色器
            GLES30.glAttachShader(mCircleProgram, mSeaVertexShaderPointer);
            checkGlError("glAttachShader");
            //链接程序
            GLES30.glLinkProgram(mCircleProgram);
            //<<<<<<<<<<<<
            //存放链接成功program数量的数组
            int[] linkStatus = new int[1];
            //获取program的链接情况
            GLES30.glGetProgramiv(mCircleProgram, GLES30.GL_LINK_STATUS, linkStatus, 0);
            //若链接失败则报错并删除程序
            if (linkStatus[0] != GLES30.GL_TRUE) { //step 2: 检查是否链接并附加成功，不成功要清理
                Log.e("ES30_ERROR", "Could not link program: ");
                Log.e("ES30_ERROR", GLES30.glGetProgramInfoLog(mCircleProgram));
                GLES30.glDeleteProgram(mCircleProgram);
                mCircleProgram = 0;
            }
        }
        //获取程序中顶点位置属性引用"指针"
        mObjectPositionPointer = GLES30.glGetAttribLocation(mCircleProgram, "objectPosition");
        //纹理采样坐标
        mVTexCoordPointer = GLES30.glGetAttribLocation(mCircleProgram, "vTexCoord");
        //获取程序中顶点颜色属性引用"指针"
        mObjectVertColorArrayPointer = GLES30.glGetAttribLocation(mCircleProgram, "objectColor");
        //获取程序中总变换矩阵引用"指针"
        muMVPMatrixPointer = GLES30.glGetUniformLocation(mCircleProgram, "uMVPMatrix");
        //半径指针
        mRPointer = GLES30.glGetUniformLocation(mCircleProgram, "r");
        //目标位置
        mTargetPointer = GLES30.glGetUniformLocation(mCircleProgram, "targetST");
        //设置分辨率指针，告诉gl脚本现在的分辨率
        mResoulutionPointer = GLES30.glGetUniformLocation(mCircleProgram, "resolution");
    }
    ...
    public void drawTo(float[] cameraMatrix, float[] projMatrix) {
        ...
        GLES30.glUseProgram(mCircleProgram);
        //设置图像分辨率
        ...
        if (mPointBuf != null && mColorBuf != null) {
            ...
            GLES30.glUniform1f(mRPointer, 0.2f); //设置半径
            GLES30.glUniform2fv(mTargetPointer, 1, new float[] {0.5f, 0.5f}, 0); //设置位置
...
//        super.drawTo(programID, positionPointer, colorPointer, cameraMatrix, projMatrix, muMVPMatrixPointer, glFunChoicePointer);
    ...
    public void destroy() {
        if (!mIsDestroyed) {
            //去除特殊shader程序
            destroyShader(mCircleProgram, mSeaVertexShaderPointer, mWaveFragShaderPointer);
            mContext = null;
        }
        mIsDestroyed = true;
    }
