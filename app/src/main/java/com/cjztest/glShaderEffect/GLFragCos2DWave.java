...
public class GLFragCos2DWave extends GLLine {
    ...
    public GLFragCos2DWave(int baseProgramPointer, float x, float y, float z, float w, float h, int windowW, int windowH, Context context) {
    ...
    private void startBindTexture() {
        //特殊纹理，需要专门加载其他程序:
        String fragShaderScript = ShaderUtil.loadFromAssetsFile("fragColorEffect1/fragShaderCos2DWave.shader", mContext.getResources());
...
//        super.drawTo(programID, positionPointer, colorPointer, cameraMatrix, projMatrix, muMVPMatrixPointer, glFunChoicePointer);
    ...
    public void destroy() {
        if (!mIsDestroyed) {
            //去除特殊shader程序
            destroyShader(mSeaProgram, mSeaVertexShaderPointer, mWaveFragShaderPointer);
            mContext = null;
        }
        mIsDestroyed = true;
    }
