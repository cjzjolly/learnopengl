...
public class GLFragEffectFlower extends GLLine {
    ...
    private void startBindTexture() {
        //特殊纹理，需要专门加载其他程序:
        String fragShaderScript = ShaderUtil.loadFromAssetsFile("fragColorEffect1/fragShaderShowLove.shader", mContext.getResources());
        ...
        mFlowerFragShaderPointer = loadShader(GLES30.GL_FRAGMENT_SHADER, fragShaderScript);
        ...
        if (mSeaProgram != 0) { //step 1: 创建program后附加编译后的脚本
            //>>>>>>>>>>>>
            //向程序中加入顶点着色器
            GLES30.glAttachShader(mSeaProgram, mFlowerFragShaderPointer);
...
//        super.drawTo(programID, positionPointer, colorPointer, cameraMatrix, projMatrix, muMVPMatrixPointer, glFunChoicePointer);
    ...
    public void destroy() {
        if (!mIsDestroyed) {
            //去除特殊shader程序
            destroyShader(mSeaProgram, mSeaVertexShaderPointer, mFlowerFragShaderPointer);
            mContext = null;
        }
        mIsDestroyed = true;
    }
